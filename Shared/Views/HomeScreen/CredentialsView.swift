...
struct CredentialsView: View {
    ...
    var body: some View {
        VStack {
            if let prefix = credentials.prefix?.uppercased() {
    ...
    private var detailsView: some View {
        ZStack {
            ...
            VStack() {
                if let universityName = credentials.universityName?.uppercased() {
                    detailsText(detail: universityName)
                }
                if let dissertationType = credentials.dissertationType?.uppercased() {
                    detailsText(detail: dissertationType)
                }
                if let studentID = credentials.studentID?.uppercased() {
